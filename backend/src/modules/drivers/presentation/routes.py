...
async def list_drivers(
    ...
    return await DriverService(db).list(
        params=pagination, search=search, status=status, is_active=is_active,
    )
...
async def update_driver(
    driver_id: uuid.UUID, data: UpdateDriverRequest,
...
async def delete_driver(
    ...
    db.add(AuditLog(
        user_id=current_user.id, action="driver.deleted",
        entity_type="driver", entity_id=str(driver_id),
        detail=f"Driver {driver.full_name} permanently deleted by admin",
    ))
