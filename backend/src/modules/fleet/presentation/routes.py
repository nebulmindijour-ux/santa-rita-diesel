...
async def list_vehicles(
    ...
    return await VehicleService(db).list(
        params=pagination, search=search, vehicle_type=vehicle_type,
...
async def update_vehicle(
    vehicle_id: uuid.UUID, data: UpdateVehicleRequest,
...
async def delete_vehicle(
    ...
    db.add(AuditLog(
        user_id=current_user.id, action="vehicle.deleted",
        entity_type="vehicle", entity_id=str(vehicle_id),
        detail=f"Vehicle {vehicle.plate} permanently deleted by admin",
    ))
